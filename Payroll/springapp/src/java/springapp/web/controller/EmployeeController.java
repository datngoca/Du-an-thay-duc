...
public class EmployeeController {
    ...
    public String addEmployee(@ModelAttribute("employee") Employee employee, ModelMap model) {
        try {
            ...
            return "admin/addEmployee"; // Hiển thị lại trang addEmployee để xem thông báo
        } catch (Exception e) {
            e.printStackTrace();
        ...
        if (userSession != null) {
            try {
                ...
                if (existingEmployee != null) {
                    ...
                    session.getTransaction().commit();
                    model.addAttribute("successMessage", "Employee EDIT successfully!");
        ...
        if (userSession != null) {
            try {
                ...
                if (employee != null) {
                    ...
                    // Kiểm tra xem thông tin nhân viên có thay đổi không
                    // Nếu không có sự thay đổi, hiển thị thông báo yêu cầu nhập thông tin
                    if (!hasEmployeeChanged(employee)) {
                        model.addAttribute("warningMessage", "Please enter employee information to edit.");
                    }
...
private boolean hasEmployeeChanged(Employee existingEmployee) {
    // So sánh các trường thông tin của nhân viên với thông tin từ form
    // Trong trường hợp này, chúng ta có thể so sánh các trường tùy theo yêu cầu
    // Ví dụ: so sánh lastName, firstName, ssn, payRate, ...
    // Ở đây chỉ là ví dụ đơn giản, bạn có thể mở rộng và cải thiện logic này
    return !existingEmployee.getLastName().equals("") ||
           !existingEmployee.getFirstName().equals("") ||
           existingEmployee.getSsn() != 0 ||
           !existingEmployee.getPayRate().equals("");
    }
}
